...
from processor.loss import sparse_categorical_crossentropy,ignore_index
...
def train_one_step(args,model,loss_fn,optimizer,batch,accumulator,first_step,take_step,global_step,clip_norm=1.0):
    ...
    with tf.GradientTape() as tape:
        ...
        total_loss = tf.reduce_mean(loss_fn(y, logits))
...
def train(args,
    ...
    loss_fn = lambda y_true,y_pred:sparse_categorical_crossentropy(y_true,y_pred,from_logits=True)
...
def evaluate(args,model,eval_dataset,mode,global_step=None):
    ...
    for batch in progress_bar(eval_dataset):
        ...
        logits = model(x).logits
        tmp_eval_loss = loss_fn(labels, logits).numpy().mean()
