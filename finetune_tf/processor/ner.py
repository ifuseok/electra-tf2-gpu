...
def ner_load_and_cache_examples(args, tokenizer, mode):
    ...
    if os.path.exists(cached_features_file):
        ...
        inputs_dict = {}
        inputs_dict["input_ids"] = [f.input_ids for f in features]
        inputs_dict["attention_mask"] = [f.attention_mask for f in features]
        if args.model_type not in ["distilkobert", "xlm-roberta"]:
            inputs_dict["token_type_ids"] = [f.token_type_ids for f in features]
        all_label_ids = [f.label_ids for f in features]

        dataset = tf.data.Dataset.from_tensor_slices((dict(inputs_dict),all_label_ids))
        logger.info("Saving features into cached file %s", cached_features_file)
        tf.data.experimental.save(dataset,cached_features_file)
    if mode == "train":
        dataset = dataset.batch(args.train_batch_size).shuffle(args.train_batch_size * 10,seed=args.seed)
    else:
